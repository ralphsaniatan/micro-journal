import { getSharedTags } from "@/app/share/actions";
import Link from "next/link";
import { ArrowLeft, User } from "lucide-react";
import { notFound } from "next/navigation";

export default async function SharedMentionsPage(
    props: {
    ...
    const tags = await getSharedTags(token);
        ...
        <main className="flex min-h-screen justify-center bg-black text-white">
            <div className="w-full max-w-xl border-x border-gray-800 min-h-screen p-4">
                <header className="flex items-center gap-4 mb-8">
                    <Link href={`/share/${token}`} className="p-2 hover:bg-white/10 rounded-full transition-colors">
                ...
                <div className="space-y-2">
                    {mentions.length === 0 ? (
                        ...
                        mentions.map((item) => (
                            <Link
                                key={item.tag}
                                href={`/share/${token}?tag=${encodeURIComponent(item.tag)}`}
